...
__itt_string_handle* pshHandlePicMessage = __itt_string_handle_create("Handle Arduino Msg");
...
/////////////////////////////////////////////////////////////////////////////
void HandlePicMessage(char *CmdResponseBuf, int nResponseLength)
{
...
// Need to figure out what to do for Carbot
	...
	//-TAL_SCOPED_TASK_NAMED("HandlePicMessage");
...
//	DWORD fdwCommMask;
	...
	char	*pCmdResponse;
	char	SioBuffer[PIC_SIO_BUF_LEN];
	char	CmdResponseBuf[PIC_SIO_BUF_LEN];
	BOOL	bFirstPacket;
	...
	memset( CmdResponseBuf, '\0', PIC_SIO_BUF_LEN );	// clean out garbage chars
...
*/
		...
		if(!ReadFile(hCommPort,		// where to read from (the open comm port)
				 ...
				 (PIC_SIO_BUF_LEN-1),	// number of bytes to read
...
*/
	...
	//						ROBOT_LOG( TRUE, "Start:HandlePicMessage\n")
							
							//Sleep(5);
							{
								///TAL_Event("Arduino Msg Received!");
								///TAL_SCOPED_TASK_NAMED("Call  HandlePicMessage");
								__itt_task_begin(pDomainArduinoThread, __itt_null, __itt_null, pshHandlePicMessage);
								HandlePicMessage(CmdResponseBuf, nCmdResponseLen);
								__itt_task_end(pDomainArduinoThread);
							}
	//						ROBOT_LOG( TRUE, "End:HandlePicMessage\n")
...
////////////////////////////////////////////////////////////////////////////////////////
	/***
__itt_string_handle* pshKobukiReadThreadLoop = __itt_string_handle_create("Kobuki Read Loop");
__itt_string_handle* pshKobukiParseBuffer = __itt_string_handle_create("Kobuki Parse");
DWORD WINAPI KobukiCommReadThreadFunc(LPVOID lpParameter) // g_hMotorCommPort
{
	IGNORE_UNUSED_PARAM (lpParameter);
	__itt_thread_set_name( "Kobuki Read Thread" );

//	HANDLE hCommPort = (HANDLE)lpParameter;	// Use Global instead
//	DWORD	fdwCommMask;
	DWORD	dwSIOBytesReceived = 0;
	unsigned char *KobukiSIOBuf = new unsigned char[KOBUKI_READ_BUF_SIZE];
	memset( KobukiSIOBuf,0,KOBUKI_READ_BUF_SIZE );

	// Create the Kobuki Parser object
	CKobukiParser *pKobukiParser = new CKobukiParser;
	...
	while(g_hMotorCommPort != INVALID_HANDLE_VALUE)
	{
		///TAL_Event("Laser Read Loop Top");
		///TAL_SCOPED_TASK_NAMED("Laser Read Loop");
		__itt_task_begin(pDomainMotorThread, __itt_null, __itt_null, pshKobukiReadThreadLoop);

		dwBytesReceived = 0;
		KobukiSIOBuf[0] = '\0';
		//memset( KobukiSIOBuf,0,KOBUKI_READ_BUF_SIZE );	// DEBUG
		...
		BOOL bSucess = 0;
		{
			...
			bSucess = ReadFile(g_hMotorCommPort,		// where to read from (the open comm port)
					 KobukiSIOBuf,						// where to put the character
					 (KOBUKI_READ_BUF_SIZE-1),			// number of bytes to read
					 &dwSIOBytesReceived,				// how many bytes were actually read
					 NULL);								// no overlapped I/O
		}
		if( !bSucess ) 
		{
			...
			DWORD dwCommError = 0;
			ClearCommError(g_hMotorCommPort, &dwCommError, NULL);
			ReportCommError("Reading Kobuki Comm port.", dwCommError );
		}
		else if( 0 != dwSIOBytesReceived )
		{
			//ROBOT_LOG( TRUE,"iROBOT Read Thread: %d bytes received\n", dwSIOBytesReceived )
			...
			// Got some data.  Parse it.
			__itt_task_begin(pDomainMotorThread, __itt_null, __itt_null, pshKobukiParseBuffer);
			if( pKobukiParser->ParseBuffer(KobukiSIOBuf, dwSIOBytesReceived) )
			{
				// Recieved binary status
				g_MotorSubSystemStatus = SUBSYSTEM_CONNECTED;
			}
			...
			__itt_task_end(pDomainMotorThread);  // pshKobukiParseBuffer
		}

		__itt_task_end(pDomainMotorThread);  // pshLaserReadThreadLoop
		Sleep(30); // TODO!!! don't spin in a tight loop!  Kobuki sends data every 15ms - IS THIS OK?  DOES ReadFile sleep?
	}	// while port open


	ROBOT_LOG( TRUE,"iROBOT COMM Read Thread exiting.\n")
	SAFE_DELETE( pKobukiParser );
	SAFE_DELETE( KobukiSIOBuf );
	return 0;
}
