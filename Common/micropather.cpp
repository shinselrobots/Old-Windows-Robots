...
class OpenQueue
{
  public:
	...
	{ 
		...
		sentinel->Init( 0, 0, PATHER_MAX, PATHER_MAX, 0 );
		sentinel->totalCost = PATHER_MAX;
	...
	void Push( PathNode* pNode )
	{
		...
		ROBOT_ASSERT( pNode->inOpen == 0 );
		ROBOT_ASSERT( pNode->inClosed == 0 );
		...
		// a value of PATHER_MAX, so it should always be sorted in.
		ROBOT_ASSERT( pNode->totalCost < PATHER_MAX );
		...
		ROBOT_ASSERT( pNode->inOpen );	// make sure this was actually added.
			#ifdef DEBUG
				sentinel->CheckList();
			#endif
		#else
	...
	{
		#ifdef USE_LIST
		ROBOT_ASSERT( sentinel->next != sentinel );
...
void MicroPather::DumpHashTable()
{
	for( int i=0; i<HASH_SIZE; ++i )
	{
    	...
    	if ( count != 0 )
    	   TRACE( "Hashtable[%3d] has %d items\n", i, count );
    }    
}    
...
PathNode* MicroPather::NewPathNode( void* state, float costFromStart, float estToEnd, PathNode* parent )
{
	...
	#ifdef USE_BINARY_HASH
	if ( state > up->state ) {
		ROBOT_ASSERT( up->right == 0 );
		up->right = pNode;
	}
	else {
		ROBOT_ASSERT( up->left == 0 );
		up->left = pNode;
	}
	#else
	up->left = pNode;
...
NodeCost* MicroPather::GetNodeNeighbors( PathNode* node, std::vector< StateCost >* neighbor, std::vector< NodeCost >* neighborNode )
{
		 ...
		 || node->numAdjacent > PathNode::MAX_CACHE )
	{
		...
		#ifdef DEBUG
		{
			// If this ROBOT_ASSERT fires, you have passed a state
			// as its own neighbor state. This is impossible --
			// bad things will happen.
			for ( unsigned i=0; i<neighbor->size(); ++i )
				ROBOT_ASSERT( (*neighbor)[i].state != node->state );
		}
		#endif
...
void MicroPather::DumpStats()
{
	...
	TRACE( "HashTableEntries=%d/%d PathNodeBlocks=%d [%dk] PathNodes=%d SolverCalled=%d\n",
...
//		ReuseAll();
	...
	while ( !open.Empty() )
	{
		...
		if( CalcuationPasses > 10 )
		{
			int UserSelection = AfxMessageBox("Path Calculation taking a long time, Abort?", MB_YESNO, 0);
			if( IDYES == UserSelection )
			{
				g_PostStatus( "GridMode Path Aborted by User!" );
				#ifdef DEBUG_PATH
				DumpStats();
				#endif
				return NO_SOLUTION;		
			}
			CalcuationPasses = 0;	// reset the counter agin
		}
		...
		else
		{
			...
			for( int i=0; i<node->numAdjacent; ++i )
			{
				...
				ROBOT_ASSERT( !( inOpen && inClosed ) );
				PathNode* inEither = inOpen ? inOpen : inClosed;

				ROBOT_ASSERT( inEither != node );
				...
				{
					ROBOT_ASSERT( !inEither );
					ROBOT_ASSERT( nodeCost[i].node );
